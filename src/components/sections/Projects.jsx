...
const Projects = () => {
  ...
  return (
    ...
    <div ref={containerRef} className="relative w-full" style={{ height: `${visibleProjects.length * 100}vh` }}>
      ...
      {/* Inner section stays sticky on the screen perfectly */}
      <section
        id="projects"
        className="sticky top-0 h-screen w-full bg-light-100 dark:bg-gradient-to-br from-[#0f0f14] via-[#12121a] to-[#0c0c10] overflow-hidden flex flex-col justify-center"
      >
        <BackgroundParticles />
        ...
        <div className="relative z-10 w-full h-full flex flex-col justify-between pt-16 md:pt-24 pb-8">
          
          {/* Header Section */}
          <div className="shrink-0 mb-4 md:mb-8">
            <h2 className="text-4xl md:text-5xl font-bold text-center mb-2 text-gray-800 dark:text-white px-4">
              Featured <span className="text-secondary-500">Projects</span>
            </h2>
            <p className="text-center text-sm text-gray-600 dark:text-gray-400 px-4">
              Scroll down or swipe to explore
            </p>
          </div>

          {/* Desktop Layout Wrapper */}
          <div className="flex-1 w-full flex flex-col lg:flex-row items-center justify-center relative px-4 md:px-12 max-w-[1600px] mx-auto min-h-[400px]">
            ...
            <div className="hidden lg:flex flex-col w-1/4 pr-8 z-20 justify-center transition-all duration-500">
              ...
              <p key={`desc-${activeProject.id}`} className="text-gray-600 dark:text-gray-300 font-medium text-sm xl:text-base leading-relaxed line-clamp-4 mb-8 animate-fade-in-up" style={{animationDelay: '100ms'}}>
              ...
              <div key={`links-${activeProject.id}`} className="flex flex-col gap-4 animate-fade-in-up" style={{animationDelay: '200ms'}}>
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-6 py-3 w-full rounded-xl bg-white/10 dark:bg-white/5 hover:bg-secondary-500 hover:text-white border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white transition-all duration-300 font-bold backdrop-blur-md"
                >
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-2 px-6 py-3 w-full rounded-xl bg-gray-900 dark:bg-black/50 text-white hover:bg-primary-500 transition-all duration-300 font-bold border border-transparent dark:border-white/10 backdrop-blur-md"
                >
            ...
            {/* 3D Carousel */}
            <div className="w-full lg:w-2/4 h-[350px] sm:h-[400px] lg:h-[450px] relative z-10 flex items-center justify-center">
              <Swiper
              ...
              >
                {visibleProjects.map((project, index) => {
                  ...
                  return (
                    <SwiperSlide
                      key={project.id}
                      className="w-[75vw] sm:w-[500px] md:w-[600px] lg:w-[100%] h-[90%] flex items-center justify-center relative"
                    >
                      <div
                        className={`relative w-full h-full rounded-3xl overflow-hidden shadow-2xl transition-all duration-500 border border-white/10 ${
                          isActive ? "ring-2 ring-secondary-500/50" : ""
                        }`}
                      >
                        <ImageWithSkeleton
                          src={project.image}
            ...
            <div className="hidden lg:flex flex-col w-1/4 pl-8 z-20 justify-center transition-all duration-500">
              <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-6">
                Technologies Used
              </h4>
              <div key={`tech-${activeProject.id}`} className="flex flex-col gap-3 animate-fade-in-up">
                {activeProject.technologies.slice(0, 5).map((tech, idx) => (
                  <div
                    key={idx}
                    className="px-5 py-3 text-sm xl:text-base font-bold text-gray-800 dark:text-white bg-white/40 dark:bg-black/40 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm hover:border-secondary-500/50 transition-colors"
                  >
                    {tech}
                  </div>
                ))}
              ...
              <RouterLink
                ...
                className="mt-8 flex items-center gap-2 text-secondary-500 font-bold hover:text-secondary-400 transition-colors group w-max"
              >
                View Full Details
          ...
          <div className="lg:hidden w-full px-6 mt-4 flex flex-col gap-4 max-w-[600px] mx-auto z-20 shrink-0">
            <h3 className="text-2xl font-bold text-gray-800 dark:text-white font-heading text-center">
              {activeProject.title}
            </h3>
            ...
            <div className="flex flex-wrap gap-2 justify-center mb-2">
              {activeProject.technologies.slice(0, 4).map((tech, idx) => (
                <span key={idx} className="px-3 py-1 text-xs font-semibold bg-white/10 dark:bg-black/30 border border-gray-200 dark:border-white/10 rounded-full dark:text-gray-200">
                  {tech}
                </span>
              ))}
            </div>
            <div className="flex gap-4 justify-center">
              <button
                onClick={() => handleLiveClick(activeProject.liveLink)}
                className="flex items-center justify-center gap-2 px-4 py-2 flex-1 rounded-xl bg-secondary-500 text-white font-bold text-sm shadow-lg"
              >
                <Monitor className="h-4 w-4" /> Live
              </button>
              <a
                href={activeProject.codeLink}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 px-4 py-2 flex-1 rounded-xl bg-gray-900 text-white font-bold text-sm shadow-lg"
              >
          ...
          <div className="w-full text-center mt-6 pb-2 relative z-10 shrink-0">
            <RouterLink
              to="/projects"
              className="
                relative inline-flex items-center gap-2 px-8 py-3
                rounded-full font-bold tracking-wide text-sm
